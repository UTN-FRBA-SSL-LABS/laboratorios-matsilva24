...
> R:garantiza que el carrito arranca en un estado predecible y válido, y es importante llamarla siempre antes de operar sobre la estructura para evitar comportamiento indefinido causado por memoria sin inicializar.
...
> R:falla ya que el valor esperado es 700 y el que obtuvo fue 350, "[FAIL] carrito_total(&c) => esperado 700, obtenido 350  (linea 47)"
...
> R: en la linea 24 esta el bug, El código suma solamente el precio de cada producto una vez, sin tener en cuenta cuántas unidades de ese producto hay (el campo cantidad del Producto)

**P7** — ¿Qué debería hacer esa línea para calcular el total correctamente?

> R: Debería multiplicar el precio por la cantidad de ese producto, ya que el total de cada ítem del carrito es precio × cantidad, no solo precio:
...
> R:falla, devuelve 1 en vez de 0 que era el esperado
...
> R: El operador incorrecto es (<=),  Debería ser (<)  Con c->cantidad <= MAX_ITEMS, cuando el carrito ya tiene MAX_ITEMS (4) productos. los índices válidos van de 0 a MAX_ITEMS - 1, es decir de 0 a 3
...
> R:Un test unitario prueba una función de forma aislada, con entradas y salidas controladas, para verificar que esa pieza puntual funciona bien por sí sola. Un test de integración prueba que varias funciones/módulos trabajan bien en conjunto. El bug de carrito_total lo detectó primero un test unitario
...
> R:En C los arreglos no tienen chequeo de límites en runtime, así que escribir fuera del arreglo corrompe memoria silenciosamente. En Python/Java el runtime chequea los índices y lanza una excepción controlada en vez de corromper memoria.
...
> R:Escribir tests obliga a definir el resultado esperado y ejecutar el código de verdad.  Detecta bugs sutiles y casos borde que a ojo se pasan por alto.
...
> R: Con cantidad = 1, precio * cantidad y precio solo dan el mismo resultado (350). El bug solo se nota cuando cantidad ≠ 1.
